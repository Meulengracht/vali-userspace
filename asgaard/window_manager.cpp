...
namespace Asgaard {
    ...
    WindowManager::WindowManager()
        : m_Client(nullptr), m_Running(false) { }
    ...
    void WindowManager::ExternalEvent(enum WmEvent event, WmEventData data)
    {
        switch (event)
        {
            case WM_SYNC:
...
extern "C"
{
    ...
    {
        // Get object that sent the sync, if none sent it, then it must been
        // a global sync
        Asgaard::Object* object = Asgaard::OM[input->serial];
        if (!object) {
            Asgaard::WM.ExternalEvent(Asgaard::WindowManager::WM_SYNC, input);
            return;
        }
        
        // publish to object
        object->ExternalEvent(Asgaard::Object::SYNC);
    }

    void wm_core_event_error_callback(struct wm_core_error_event* input)
    {
        Asgaard::Object* object = Asgaard::OM[input->object_id];
        if (!object) {
            // Global error, this must be handled on application level
            Asgaard::WM.ExternalEvent(Asgaard::WindowManager::WM_ERROR, input);
            return;
        }
        
        // publish error to object
        object->ExternalEvent(Asgaard::Object::ERROR, input);
    }
    ...
    void wm_core_event_object_callback(struct wm_core_object_event* input)
    {
        switch (input->type)
        {
            ...
            case object_type_screen: {
                Asgaard::Screen* screen = Asgaard::OM.CreateServerObject<Asgaard::Screen>(input->object_id);
            } break;
            
            // Handle client completion objects
            default: {
                Asgaard::Object* object = Asgaard::OM[input->object_id];
                object->ExternalEvent(Asgaard::Object::CREATION, input);
            } break;
        }
    }
    ...
    {
        Asgaard::Object* object = Asgaard::OM[input->screen_id];
        if (!object) {
            // log
            return;
        }
        ...
        auto screen = dynamic_cast<Asgaard::Screen*>(object);
        if (!screen) {
            // log
            return;
        }
        ...
        // publish to object
        object->ExternalEvent(Asgaard::Object::SCREEN_PROPERTIES, input);
    }
    ...
    void wm_screen_event_mode_callback(struct wm_screen_mode_event* input)
    {
        ...
        // publish to object
        object->ExternalEvent(Asgaard::Object::SCREEN_MODE, input);
    }
    
    // SURFACE PROTOCOL EVENTS
    void wm_surface_event_format_callback(struct wm_surface_format_event* input)
    {
        Asgaard::Object* object = Asgaard::OM[input->surface_id];
        if (!object) {
            // log
            return;
        }
        ...
        auto window = dynamic_cast<Asgaard::WindowBase*>(object);
        if (!window) {
            // log
            return;
        }
        ...
        // publish to object
        object->ExternalEvent(Asgaard::Object::SURFACE_FORMAT, input);
    }
    ...
    void wm_surface_event_resize_callback(struct wm_surface_resize_event* input)
    {
        ...
        // publish to object
        object->ExternalEvent(Asgaard::Object::SURFACE_RESIZE, input);
    }
    
    // BUFFER PROTOCOL EVENTS
    void wm_buffer_event_release_callback(struct wm_buffer_release_event* input)
    {
        Asgaard::Object* object = Asgaard::OM[input->buffer_id];
        if (!object) {
            // log
            return;
        }
        ...
        // verify object is a window object
        auto buffer = dynamic_cast<Asgaard::WindowBase*>(object);
        if (!buffer) {
            // log
            return;
        }
        ...
        // publish to object
        object->ExternalEvent(Asgaard::Object::BUFFER_RELEASE, input);
    }
}
